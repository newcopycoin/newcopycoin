...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeNewCopyCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeNewCopyCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeNewCopyCoinAmount = strAnonymizeNewCopyCoinAmount.remove(strAnonymizeNewCopyCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNewCopyCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeNewCopyCoinAmount*COIN) nMaxToAnonymize = nAnonymizeNewCopyCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeNewCopyCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeNewCopyCoinAmount));
        strAnonymizeNewCopyCoinAmount = strAnonymizeNewCopyCoinAmount.remove(strAnonymizeNewCopyCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNewCopyCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeNewCopyCoinAmount == 0){
