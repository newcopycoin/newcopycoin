...
int nAnonymizeNewCopyCoinAmount = 1000;
...
#else
    const char* pszModule = "newcopycoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NewCopyCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NewCopyCoin
    // Mac: ~/Library/Application Support/NewCopyCoin
    // Unix: ~/.newcopycoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NewCopyCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "newcopycoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "newcopycoind.pid"));
